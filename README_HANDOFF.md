...
- `cad.read_objects`의 `scope=window/range` 좌표는 반드시 PowerShell에서 `return ,([double[]]@(...))` 형태로 반환해야 합니다. 쉼표가 빠지면 PowerShell이 배열을 풀어 AutoCAD COM `SelectionSet.Select`에 `Object[]`처럼 전달하고 `Invalid argument Point1 in Select`가 재발합니다.
- 최신 검증 기준: AutoCAD 임시 도면에서 `cad.read_objects(scope=window)`가 TEXT 2개 handle을 읽고, 그 handle을 `cad.renumber_selected_text`로 넘겨 미리보기는 변경 없음, 적용은 `CF-01`/`CF-02`로 변경되는 것까지 확인했습니다.
