...
찾은 텍스트의 도곽 기준 상대 위치를 저장하고, 다른 DWG와 다른 도곽에서는 같은 상대 위치 주변의 Text/MText를 도면번호로 인식합니다. 검색 대상 문자는 `Text`와 `MText`로 고정하고, 상대 위치 허용 오차는 너무 넓은 오인식을 피하기 위해 `5` DWG 단위로 고정합니다.
...
설정 체크박스가 아니라 실행 버튼을 두 단계로 분리합니다.

- `분석 미리보기`: 원본 DWG를 저장하지 않고 도곽 수, 파일별 번호 범위, 변경 전/후 도면번호 목록만 계산합니다.
- `원본에 적용`: 분석 미리보기 결과가 생성된 뒤에만 활성화합니다. 사용자가 원본 DWG 직접 수정 경고를 확인해야 실제 수정이 실행됩니다.
...
- 검색 대상과 허용 오차: 검색 대상 문자는 `Text`/`MText`, 상대 위치 허용 오차는 `5` DWG 단위로 고정합니다.
- 새 도면번호 규칙: 접두어, 시작번호, 자리수. 증가값은 항상 1로 고정합니다.
- 실행 동작: `분석 미리보기` 버튼은 저장 없이 결과만 계산하고, `원본에 적용` 버튼은 미리보기 확인 후 저장합니다.
- 원본 적용 안전 동작: 백업 정책은 `백업 없음`, 최종 확인은 `필수`로 고정합니다.
...
- Parameter mapping:
  ...
  - 고정 도곽 순서 `좌상단 → 우하단` → 도곽 내부 순번 기준
  - 고정 검색 대상 `Text/MText`, 고정 상대 위치 허용 오차 `5` → 도면번호 텍스트 매칭 기준
  - `settings.number_prefix`, `settings.start_number`, `settings.digit_count`, 고정 증가값 `1` → 새 도면번호 생성 규칙
  - `runtime.action == "preview"` → `cad.update_text_values`를 저장 없는 미리보기 모드로 실행
  - 고정 백업 정책 `none`, 고정 최종 확인 `true` → 원본 수정 안전 정책
...
- `원본에 적용` 전에는 `분석 미리보기` 결과가 생성되어야 합니다.
...
- 입력: `A.dwg`, `B.dwg`
- 설정: 접두어 `P-`, 시작번호 `101`, 자리수 `3`, 도곽 순서 `좌상단 → 우하단`
